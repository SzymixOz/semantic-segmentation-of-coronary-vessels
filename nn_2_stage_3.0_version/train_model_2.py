...
class CoronarySmallDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.resize(image, (256, 256), interpolation=cv2.INTER_NEAREST_EXACT)
        
        mask = cv2.imread(mask_path, cv2.IMREAD_UNCHANGED)
        # mask = cv2.cvtColor(mask, cv2.COLOR_RGBA2RGB)
        mask = cv2.resize(mask, (256, 256), interpolation=cv2.INTER_NEAREST_EXACT)
...
train_model(model, train_loader, val_loader, criterion, optimizer,
            "RGB_model_INTER_NEAREST_EXACT", num_epochs=60, early_stopping=3)
